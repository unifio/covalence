  ...
  class TerraformCli
    ...
    def self.terraform_check_style(path)
      if Covalence::TF_IMG.blank?
        ...
        output, status = Open3.capture2e(ENV, "#{Covalence::TF_CMD} -v #{parent}:/data -w /data/#{base} #{Covalence::TF_IMG} fmt -write=false")
      end
          ...
          if sub_hash.blank?
            ...
            define_singleton_method(terraform_cmd) do |path=Dir.pwd(), args: ''|
              if Covalence::TF_IMG.blank?
                ...
                parent, base = Pathname.new(path).split
                PopenWrapper.run([Covalence::TF_CMD, "-v #{parent}:/data -w /data/#{base} #{Covalence::TF_IMG}", cmd], '', args)
              end
            end
            ...
            sub_hash.keys.each do |sub_command|
              ...
              define_singleton_method(terraform_cmd) do |path=Dir.pwd(), args: ''|
                if Covalence::TF_IMG.blank?
                  ...
                  parent, base = Pathname.new(path).split
                  PopenWrapper.run([Covalence::TF_CMD, "-v #{parent}:/data -w /data/#{base} #{Covalence::TF_IMG}", cmd, sub_command], '', args)
                end
              end
            end
          else
